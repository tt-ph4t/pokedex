...
const EvolutionChainTree = async ({ chain, url }) => {
  ...
  return evolutionChainTree(
    chain ?? (await Pokedex.api.getResource(url)).data.chain,
  );
};

export default mapValues(
  {
    ...Pokedex.api.route.names.reduce((a, b) => {
      a[b] = {};
  ...
  (value) => {
    ...
    return {
      ...
      render: async (...args) => (await value.render(...args)) ?? NO_CONTENT,
    };
  },
);
